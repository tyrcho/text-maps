...
away below). Positioned exactly like any other room feature (see **Positioning** below) — add a trailing
`col,row` or wall-side word to place it somewhere other than the room's centre, e.g. `stairs: up west 0,0`
or `stairs: up 2,1` (position with no explicit facing, which then stays north); this is what lets two
staircases coexist in one room without overlapping. `spiral-stairs:`/`ladder:` take the same optional
trailing position (no facing concept for those two): `spiral-stairs: up 1,1`, `ladder: down east`.

**A curated set of common furnishing icons works with no `import` at all** — `torch:`, `chest:`, `barrel:`,
`wooden-crate:`, `ionic-column:`, `key:`, `anvil:`, `bookshelf:`, `cage:`, `campfire:`, `cauldron:`,
`sarcophagus:`, `skull-crossed-bones:`, `spider-web:`, `stone-block:`, `well:`, `beer-stein:`,
`battle-axe:`, `round-shield:`. Hand-picked from [game-icons.net](https://game-icons.net) (CC BY 3.0) and
bundled directly into the renderer as `.svg` files kept in `doc/icons/builtin/` (see that folder's
`SOURCES.md` for attribution) — unlike the general Iconify case below, these never touch the network. Same
size/position vocabulary as any other icon feature.

**Every other room feature is an Iconify icon** — anything not in the builtin set above, from any Iconify
set: statues, stalactites, pools, fireplaces, beds, curtains, arrow slits, illusory walls, and so on. Import
an icon set with a header-level `import` statement, then reference any icon in it as `<alias>.<icon-name>`:
```
map dungeon
...
- Repeating the same `<alias>.<icon-name>:` key on separate lines within a room places one instance per
  ...
  repeating `stairs:`/`spiral-stairs:`/`ladder:`, each with its own position, for more than one of those in
  a room.
