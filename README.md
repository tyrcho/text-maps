...
**Builtin icons** (`core/shared/.../icons/BuiltinIcons.scala`): a `Map[String, String]` of icon-name → path data for the curated no-`import` furnishing set (see "Positioning" above), embedded at compile time from the standalone `.svg` files kept in `doc/icons/builtin/` and rendered as `<symbol>`s in `SvgStringRenderer.defs()` — `RoomFeature.Icon("builtin", <name>, ...)` is the sentinel `DslParser` emits for a bare (non-`alias.`-prefixed) key that matches one of these names, resolved without going through `IconFetcher` at all.
